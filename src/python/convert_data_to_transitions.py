...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "input data is transposed, with index as feature IDs, column headers "
        "as sample/isolate names, and values as binary presence/absence, e.g. "
        "as generated by \"bcftools query -f '%%CHROM_%%POS[\\t%%GT]\\n'\""
        )
)
parser.add_argument(
    '-sep',
...
def LoadData(file_name, keep_nan=0, index_col=0, sep=',', transpose=False):
    df = pd.read_csv(file_name, index_col=index_col, sep=sep).astype(int)
    if transpose:
        df = df.T
...
if args.input_type == "cross-sectional":
    data = LoadData(args.data, keep_nan=1, sep=args.sep,
...
if args.input_type == "longitudinal":
    data = LoadData(args.data, keep_nan=0, index_col=None, sep=args.sep, 
                    transpose=args.transpose)
...
if args.input_type == "sample-tree":
    ...
    df = pd.read_csv(args.data, index_col=0, sep=args.sep).astype(int)
    if transpose:
        df = df.rename_axis(index=None).T
    transitions = []
...
if args.input_type == "phylogenetic":
    data = LoadData(args.data, keep_nan=0, sep=args.sep,
                    transpose=args.transpose)
